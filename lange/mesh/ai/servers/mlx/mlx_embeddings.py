...
class MLXEmbeddingsServer(InferenceServer):
    ...
    def run(self) -> None:
        ...
        async def create_embeddings(request: EmbeddingRequest) -> CreateEmbeddingResponse:
            ...
            if len(texts) == 1 and raw_embeddings and isinstance(raw_embeddings[0], float):
            ...
            token_values = input_ids.tolist() if hasattr(input_ids, "tolist") else input_ids
    ...
    def stop(self) -> None:
        """Stop the embeddings server when an external server handle exists."""
        pass
